...
# Esse índice possui a vantagem de ser pouco influenciado pelo tamanho da amostra (Beck & Schwanghart, 2010; Condit et al., 1996, 1998; Taylor et al., 1976).
# Para sua aplicação, as abundâncias devem estar em formato de contagem e, teoricamente, seguir uma distribuição log-série,
# embora o índice seja considerado robusto mesmo frente a violações desse pressuposto (Magurran, 2004).
# Ainda assim, testamos o ajuste das abundâncias à distribuição log-série em todas as combinações correspondentes ao terceiro maior fragmento.
...
name_data2<- paste0("d1_data_dez_mil_03.csv") 
...
plan(sequential)
 ...
 AICs_ls["classification"]<-"dez_mil_terceiro_maior" 
 AICs_ls["study_id"]<-name
 AICs_ls["frag_base"]<-resultado$frag_base
 AICs_ls["frag_base_area"]<-resultado$area
 
 # Salvar em .csv
 write.csv(AICs_ls, file.path(caminho_pasta2, "T03_demais_niveis_AIC_especies_dez_mil.csv"), row.names = FALSE) 
